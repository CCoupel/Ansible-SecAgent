...
func TestAdminCreateEnrollmentToken(t *testing.T) {
	...
	if !startsWith(resp.Token, "secagent_enr_") {
		t.Errorf("expected secagent_enr_ prefix, got %q", resp.Token[:10])
	}
	if resp.ID == "" {
...
func TestAdminCreatePluginToken(t *testing.T) {
	...
	resp := decodeTokenCreate(t, w)
	if !startsWith(resp.Token, "secagent_plg_") {
		t.Errorf("expected secagent_plg_ prefix, got %q", resp.Token[:10])
	}
	if resp.Role != "plugin" {
