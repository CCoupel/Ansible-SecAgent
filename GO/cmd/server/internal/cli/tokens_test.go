...
func TestTokensCreate_Enrollment_Success(t *testing.T) {
	...
	if !strings.Contains(out, "relay_enr_abc123") {
...
func TestTokensCreate_Plugin_Success(t *testing.T) {
	...
	if !strings.Contains(out, "relay_plg_xyz789") {
		t.Errorf("expected token in output, got: %s", out)
	}
}
...
func TestTokensList_All(t *testing.T) {
	...
	if strings.Contains(out, "relay_enr_") || strings.Contains(out, "relay_plg_") {
...
func TestTokensCreate_EnrollmentPrefixRelay_enr(t *testing.T) {
	mockServer(t, func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusCreated)
		json.NewEncoder(w).Encode(map[string]interface{}{
			"token":            "relay_enr_" + fmt.Sprintf("%064x", 0),
			"id":               "uuid-prefix-enr",
			"role":             "enrollment",
			"hostname_pattern": "vp.*",
			"reusable":         false,
	...
	if !strings.Contains(out, "relay_enr_") {
		t.Errorf("expected relay_enr_ prefix in output, got: %s", out)
	}
}
...
func TestTokensCreate_PluginPrefixRelay_plg(t *testing.T) {
	mockServer(t, func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusCreated)
		json.NewEncoder(w).Encode(map[string]interface{}{
			"token":      "relay_plg_" + fmt.Sprintf("%064x", 0),
			"id":         "uuid-prefix-plg",
			"role":       "plugin",
			"expires_at": "",
			"created_at": time.Now().UTC().Format(time.RFC3339),
	...
	if !strings.Contains(out, "relay_plg_") {
		t.Errorf("expected relay_plg_ prefix in output, got: %s", out)
	}
}
...
func TestTokensList_NoPlainTextEvenOnServerBug(t *testing.T) {
		...
		json.NewEncoder(w).Encode([]map[string]interface{}{
			{
				...
				"role":       "plugin",
				"token_hash": "safehash",
				"token":      "relay_plg_this_should_not_appear",
				"created_at": "2026-03-01T00:00:00Z",
			},
		})
	})
	...
	if strings.Contains(out, "relay_plg_this_should_not_appear") {
